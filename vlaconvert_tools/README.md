...
## Collect LIBERO Accuracy And Latency Results

LIBERO eval logs are written to `/tmp/logs` using only the suite name, for
example `libero_eval_libero_spatial.log`.  That means a later FakeQuant run can
overwrite an earlier RealQuant run for the same suite.  Collect results right
after each batch of experiments and pass the mode explicitly:

```bash
python vlaconvert_tools/collect_libero_results.py \
  --mode fake \
  --run-name fake_20260719 \
...
python vlaconvert_tools/collect_libero_results.py \
  --mode real \
  --run-name real_20260719 \
  --suites libero_spatial libero_object libero_goal libero_10
```

This creates:

- `results/<run>/<mode>/<suite>/libero_eval_<suite>.log`
- `results/<run>/<mode>/<suite>/libero_eval_<suite>_latency_summary.json`
- `results/<run>/<mode>/<suite>/libero_eval_<suite>_latency_steps.csv`
- `results/<run>/<mode>/<suite>/libero_eval_<suite>_latency_steps.jsonl`
- `results/<run>/<mode>/<suite>/quantvla_converted_<mode>_<suite>_replacement_report.json`
- `results/summary.csv`
- `results/summary.md`
- `results/summary.json`

The summary table focuses on:

- accuracy: `success_rate_pct`, `num_successes`, `num_episodes`
- replacement sanity: `successfully_replaced`,
  `fallback_fp16_layers_count`
- latency: `step_total_ms_*`, `client_roundtrip_ms_*`,
  `policy_total_ms_*`, `policy_model_get_action_ms_*`, `env_step_ms_*`

Use mean for the central runtime and p90/p99 to reason about tail latency.
Use each suite's raw `latency_steps.csv` for detailed outlier analysis.
...
This experiment does not yet replace GR00T's runtime linear layers with a
transform-aware vLLM GPTQ-Marlin kernel. The next integration step is:

1. Load this converted GPTQ-like checkpoint.
2. Wrap each target Linear with:
   - QuantVLA input transform
   - Marlin-backed GPTQ matmul
   - QuantVLA output restore when needed
3. Verify:
   - `Target Linear layers`
   - `Successfully replaced`
   - `Unmatched checkpoint keys`
   - `Unreplaced target layers`
   - `Fallback FP16 layers`
